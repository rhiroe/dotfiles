...
docker build --pull --secret id=aws_config,src=$HOME/.aws/config --secret id=saml2aws,src=$HOME/.saml2aws -f saml2aws.dockerfile -t saml2aws .
docker build --pull -f ecspresso.dockerfile -t ecspresso .
